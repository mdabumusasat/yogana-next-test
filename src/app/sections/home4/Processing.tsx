"use client";

import React, { useState } from "react";

const tabs = [
  {
    id: "analysis",
    title: "Business Analysis",
    heading: "Essential Features for Time Management",
    content:
      "Sed ut perspiciatis unde omnis iste natus voluptatem accusantium doloremque laudantium totamto aperiame eaque ipsa quae ab illo inventore veritatis",
    image1: "image1.jpg",
    image2: "image2.jpg",
  },
  {
    id: "strategy",
    title: "Business Strategy",
    heading: "Essential Features for Business Strategy",
    content:
      "Sed ut perspiciatis unde omnis iste natus voluptatem accusantium doloremque laudantium totamto aperiame eaque ipsa quae ab illo inventore veritatis",
    image1: "image1.jpg",
    image2: "image2.jpg",
  },
  {
    id: "execution",
    title: "Final Execution",
    heading: "Essential Features for Final Execution",
    content:
      "Sed ut perspiciatis unde omnis iste natus voluptatem accusantium doloremque laudantium totamto aperiame eaque ipsa quae ab illo inventore veritatis",
    image1: "image1.jpg",
    image2: "image2.jpg",
  },
];

const Processing = () => {
  const [activeTab, setActiveTab] = useState("analysis");

  return (
    <section id="project" className="process-section-four pt-120 pb-120">
      <div className="container">
        <div className="outer-box">
          {/* Section Title */}
          <div className="sec-title center mb-50">
            <h6
              className="sub-title wow fadeInUp"
              data-wow-delay="00ms"
              data-wow-duration="1500ms"
            >
              Process
            </h6>
            <h2
              className="title wow splt-txt"
              data-splitting
            >
              Ensure the success of <br /> your business venture.
            </h2>
          </div>

          {/* Tabs Navigation */}
          <div className="process-tab">
            <ul className="nav nav-tabs" role="tablist">
              {tabs.map((tab, idx) => (
                <li className="nav-item mb-3 mb-md-0" key={tab.id}>
                  <button
                    className={`nav-link ${activeTab === tab.id ? "active" : ""}`}
                    type="button"
                    role="tab"
                    aria-selected={activeTab === tab.id}
                    aria-controls={tab.id}
                    onClick={() => setActiveTab(tab.id)}
                  >
                    {tab.title}
                    <span className="number">{`0${idx + 1}`}</span>
                  </button>
                </li>
              ))}
            </ul>
          </div>

          {/* Tabs Content */}
          <div className="tab-content mt-30">
            {tabs.map((tab) => (
              <div
                key={tab.id}
                id={tab.id}
                role="tabpanel"
                className={`tab-pane fade ${
                  activeTab === tab.id ? "show active" : ""
                }`}
              >
                <div className="row g-5 align-items-center">
                  {/* Left Column */}
                  <div className="col-lg-7 content-column">
                    <div className="inner-column">
                      <div className="sec-title mb-30">
                        <h2 className="title">{tab.heading}</h2>
                        <p className="text fs-16">{tab.content}</p>
                      </div>
                      <div className="list">
                        <ul>
                          <li>
                            <i className="fa-solid fa-check" />
                            Optimized Sprint Planning
                          </li>
                          <li>
                            <i className="fa-solid fa-check" />
                            Continuous Improvement
                          </li>
                        </ul>
                        <ul>
                          <li>
                            <i className="fa-solid fa-check" />
                            Incremental Delivery Approach
                          </li>
                          <li>
                            <i className="fa-solid fa-check" />
                            Delivering Innovative
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>

                  {/* Right Column */}
                  <div className="col-lg-5 image-column">
                    <div className="inner-column">
                      <figure className="image">
                        <svg
                          width="462"
                          height="455"
                          viewBox="0 0 462 455"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M459.997 221.079C454.897 264.279 435.197 314.479 395.797 337.579C336.297 372.479 341.297 426.079 225.097 449.379C206.997 452.979 189.797 454.579 173.597 454.279C164.897 454.079 156.497 453.379 148.297 452.179C74.3971 441.379 24.6971 390.079 6.49711 328.179C-17.5029 246.679 30.3971 193.379 62.1971 161.179C93.9971 129.079 117.197 28.8789 231.197 5.07893C248.597 1.47893 266.397 -0.421065 284.097 0.0789348C324.697 1.07893 368.897 14.3789 399.497 42.1789C401.797 44.2789 403.997 46.4789 406.197 48.6789C448.897 92.3789 466.897 162.179 459.997 221.079Z"
                            fill="#1A4137"
                            fillOpacity="0.1"
                          />
                        </svg>
                        <div className="image1">
                          <img
                            src={`/assets/images/process/${tab.image1}`}
                            alt="Process Image 1"
                          />
                        </div>
                        <div className="image2">
                          <img
                            src={`/assets/images/process/${tab.image2}`}
                            alt="Process Image 2"
                          />
                        </div>
                      </figure>
                    </div>
                  </div>
                </div>
              </div>
            ))}
          </div>
        </div>
      </div>
    </section>
  );
};

export default Processing;
